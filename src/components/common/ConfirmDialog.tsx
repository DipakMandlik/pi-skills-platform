...
const variantConfig: Record<ConfirmVariant, { icon: React.ReactNode; iconBg: string; iconColor: string; confirmVariant: 'danger' | 'primary' }> = {
  danger: {
    ...
    iconBg: 'status-bg-red',
    iconColor: '',
    confirmVariant: 'danger',
  },
  warning: {
    icon: <AlertTriangle className="w-5 h-5" />,
    iconBg: 'status-bg-amber',
    iconColor: '',
    confirmVariant: 'primary',
  },
  info: {
    icon: <Info className="w-5 h-5" />,
    iconBg: 'status-bg-blue',
    iconColor: '',
    confirmVariant: 'primary',
  },
};
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-sm bg-[var(--color-surface-elevated)] rounded-xl shadow-2xl border border-[var(--color-border)] overflow-hidden"
          >
            <div className="p-5">
