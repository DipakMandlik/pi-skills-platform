...
import { Send, X, Bot, User, Play, Activity, Plus, SlidersHorizontal, Sparkles, Database, CheckCircle2 } from 'lucide-react';
...
export function CenterPanel({ projectButton }: { projectButton?: React.ReactNode } = {}) {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 min-w-0 relative bg-surface-elevated font-sans">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-cyan-950/10 via-slate-950 to-slate-950 pointer-events-none" />
      <div className="absolute inset-0 opacity-[0.02] pointer-events-none mix-blend-screen bg-[linear-gradient(135deg,_#fff_1px,_transparent_1px),linear-gradient(45deg,_#fff_1px,_transparent_1px)] bg-[size:16px_16px]" />

      <div className="h-12 border-b border-border flex items-center gap-4 px-4 relative z-10 bg-background/80 backdrop-blur-md shrink-0">
        {/* Title */}
        <h2 className="text-[11px] font-bold text-muted uppercase tracking-widest shrink-0">Workspace Terminal</h2>

        {/* Tab pills */}
        <div className="flex items-center gap-1 bg-surface/60 border border-border rounded-full px-1 py-1">
          {/* Project tab (active) */}
          {projectButton ? (
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1 text-[11px] font-medium text-muted hover:text-foreground rounded-full transition-colors"
          >
          ...
          {/* Analytics Mode tab */}
          <button className="flex items-center gap-1 px-3 py-1 text-[11px] font-medium text-muted hover:text-foreground rounded-full transition-colors">
            <span className="text-muted/40 mr-0.5 select-none">|</span>
            Analytics Mode
          </button>
        </div>

        {/* MCP status — subtle indicator at far right */}
        <div className="ml-auto flex items-center gap-1.5">
          <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${
            mcpServerStatus === 'ok' ? 'bg-cyan-400' :
            mcpServerStatus === 'degraded' ? 'bg-amber-400' :
            mcpServerStatus === 'error' ? 'bg-red-400' : 'bg-muted'
          }`} />
          <span className="text-[10px] font-mono text-muted/60 uppercase tracking-wide">MCP</span>
        </div>
      </div>

      <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-4 relative z-10 px-6">
        {chatHistory.map((msg) => (
          <motion.div 
          ...
          >
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 border ${
              msg.role === 'user' 
                ? 'bg-surface-hover border-slate-700 text-primary' 
                : 'bg-surface border-border text-accent shadow-[0_0_15px_rgba(99,102,241,0.05)]'
            }`}>
            ...
            <div className={`flex flex-col gap-2 ${msg.role === 'user' ? 'items-end' : 'items-start'} w-full`}>
              <div className={`px-5 py-3.5 rounded-2xl text-sm leading-relaxed ${
                msg.role === 'user' 
                  ? 'bg-surface-hover text-foreground rounded-tr-sm border border-slate-700' 
                  : 'bg-surface/80 backdrop-blur-sm border border-border text-foreground rounded-tl-sm'
              }`}>
                {msg.content}
              </div>
              
              {msg.sql && (
                <div className="w-full max-w-3xl bg-[#0a0a0a] border border-border rounded-xl overflow-hidden mt-2 shadow-[0_0_15px_rgba(0,0,0,0.5)] group">
                  <div className="flex items-center justify-between px-4 py-2 border-b border-border bg-surface/50">
                    <span className="text-[11px] font-mono font-bold text-muted uppercase tracking-widest">Generated SQL</span>
                    <button 
                      onClick={() => setGeneratedSQL(msg.sql || null)}
                      className="flex items-center gap-1.5 text-xs text-primary hover:text-primary transition-colors font-medium opacity-0 group-hover:opacity-100"
                    >
                      <Play className="w-3.5 h-3.5" />
                      View in Editor
                    </button>
                  </div>
                  <pre className="p-5 text-sm font-mono text-cyan-50/90 overflow-x-auto leading-relaxed">
                    <code>{msg.sql}</code>
                  </pre>
                </div>
              )}
              ...
              {msg.thinking && (
                <details className="w-full max-w-3xl bg-surface/50 border border-border rounded-xl p-3 text-xs text-muted hover:border-slate-700 transition-colors">
                  <summary className="cursor-pointer font-medium text-muted select-none">Execution Trace</summary>
                  <pre className="mt-3 whitespace-pre-wrap font-mono text-[11px] text-muted/80 border-t border-border/80 pt-3">{msg.thinking}</pre>
                </details>
              )}
            </div>
          </motion.div>
      ...
      <div className="px-6 pb-3 pt-2 relative z-10 sticky bottom-0 bg-gradient-to-t from-background via-background/95 to-transparent">
        <div className="max-w-3xl mx-auto relative">
          {(pendingProcedureContext || (guidedFeedback && guidedCurrentStep) || optimizerRewrite) && (
            <div className="mb-3 max-h-[38vh] overflow-y-auto space-y-3">
              {optimizerRewrite && (
                <div className="bg-[var(--color-surface-elevated)] border border-[var(--color-border)] rounded-xl p-4 shadow-sm">
                  ...
                  <div className="border border-border rounded-lg overflow-hidden mb-3">
                    <div className="px-3 py-1.5 text-[11px] uppercase tracking-wide text-[var(--color-muted-foreground)] bg-[var(--color-surface)] border-b border-[var(--color-border)]">Inline Diff</div>
                    <div className="max-h-56 overflow-auto bg-[var(--color-surface-elevated)] font-mono text-[11px]">
                      ...
                      {(['SQL', 'JAVASCRIPT', 'PYTHON'] as ProcedureLanguage[]).map((language) => (
                        <button
                          ...
                          onClick={() => setProcedureQuickConfig((prev) => ({ ...prev, language }))}
                           className={`px-2.5 py-1 rounded-md border text-xs font-medium ${procedureQuickConfig.language === language ? 'bg-[var(--color-accent-light)] border-[var(--color-accent)] text-[var(--color-accent)]' : 'bg-[var(--color-surface-elevated)] border-[var(--color-border)] text-[var(--color-foreground)] hover:bg-[var(--color-surface-hover)]'}`}
                        >
                          {language}
                        </button>
                      ))}
                      ...
                      {(['CALLER', 'OWNER'] as Array<'CALLER' | 'OWNER'>).map((mode) => (
                        <button
                          ...
                          onClick={() => setProcedureQuickConfig((prev) => ({ ...prev, executeAs: mode }))}
                           className={`px-2.5 py-1 rounded-md border text-xs font-medium ${procedureQuickConfig.executeAs === mode ? 'bg-[var(--color-accent-light)] border-[var(--color-accent)] text-[var(--color-accent)]' : 'bg-[var(--color-surface-elevated)] border-[var(--color-border)] text-[var(--color-foreground)] hover:bg-[var(--color-surface-hover)]'}`}
                        >
                          {mode}
                        </button>
                      ))}
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button
                        ...
                        onClick={() => setProcedureQuickConfig((prev) => ({ ...prev, includeLogging: true }))}
                         className={`px-2.5 py-1 rounded-md border text-xs font-medium ${procedureQuickConfig.includeLogging ? 'bg-[var(--color-accent-light)] border-[var(--color-accent)] text-[var(--color-accent)]' : 'bg-[var(--color-surface-elevated)] border-[var(--color-border)] text-[var(--color-foreground)] hover:bg-[var(--color-surface-hover)]'}`}
                      >
                        Yes
                      </button>
                      <button
                      ...
                      {(['TEXT', 'TABLE'] as Array<'TEXT' | 'TABLE'>).map((returnType) => (
                        <button
                          ...
                          onClick={() => setProcedureQuickConfig((prev) => ({ ...prev, returnStyle: returnType }))}
                           className={`px-2.5 py-1 rounded-md border text-xs font-medium ${procedureQuickConfig.returnStyle === returnType ? 'bg-[var(--color-accent-light)] border-[var(--color-accent)] text-[var(--color-accent)]' : 'bg-[var(--color-surface-elevated)] border-[var(--color-border)] text-[var(--color-foreground)] hover:bg-[var(--color-surface-hover)]'}`}
                        >
                          {returnType}
                        </button>
                      ))}
                      ...
                      return (
                        <button
                          ...
                          onClick={() => toggleGuidedSelection(option)}
                           className={`w-full text-left px-3 py-2 rounded-lg border text-sm transition-colors ${selected ? 'bg-[var(--color-accent-light)] border-[var(--color-accent)] text-[var(--color-accent)]' : 'bg-[var(--color-surface-elevated)] border-[var(--color-border)] text-[var(--color-foreground)] hover:bg-[var(--color-surface-hover)]'}`}
                        >
                          {option}
                        </button>
                      );
          ...
          <div className="w-full max-w-3xl mx-auto mb-1 space-y-2 px-0">
            {/* Context Pills */}
              ...
              <AnimatePresence>
                {activeSkills.map(skill => (
                  <motion.div 
                    ...
                    className="flex items-center gap-1.5 bg-primary/10 text-cyan-700 px-2.5 py-1 rounded-full text-[11px] font-semibold border border-primary/20"
                  >
                ...
                {selectedTables.map(table => (
                  <motion.div 
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, scale: 0.9 }}
                    className="flex items-center gap-1.5 bg-indigo-500/10 text-indigo-700 px-2.5 py-1 rounded-full text-[11px] font-semibold border border-accent/20"
                  >
                    <Database className="w-3.5 h-3.5" />
                    {splitQualifiedName(table).table}
                    <button 
                      type="button"
                      onClick={() => toggleTable(table)}
                      className="hover:bg-indigo-500/20 rounded-full p-0.5 ml-0.5 transition-colors"
                    >
                      <X className="w-3.5 h-3.5" />
                    </button>
                  </motion.div>
            ...
            <form 
              ...
              className="bg-background/95 border border-border rounded-2xl shadow-sm flex items-center px-2 py-1.5 focus-within:border-primary/40 transition-colors duration-200 relative z-20"
            >
              <div className="px-1.5 shrink-0 flex items-center justify-center">
                {isGenerating ? (
              ...
              <select
                ...
                className="shrink-0 rounded-lg border border-transparent bg-transparent px-2 py-1 text-[11px] font-medium text-primary outline-none hover:bg-surface"
                title="Model"
              >
                {MODEL_OPTIONS.map((group) => (
                  <optgroup key={group.group} label={group.group} className="bg-surface border-none">
              ...
              <textarea
                ...
                placeholder="Message π-Optimized or command data..."
                className="flex-1 bg-transparent text-foreground placeholder-slate-500 resize-none outline-none py-2 px-2 max-h-24 min-h-[36px] text-sm font-medium leading-normal self-center"
                rows={1}
              />
              
              <div className="flex items-center gap-1.5 pl-1 pr-0.5 shrink-0">
                <button
                  type="button"
                  onClick={() => setThinkingEnabled(!thinkingEnabled)}
                  className={`p-2 rounded-full transition-colors flex items-center justify-center ${thinkingEnabled ? 'bg-cyan-950/30 text-primary border border-primary/30' : 'text-muted hover:bg-surface hover:text-foreground border border-transparent'}`}
                ...
                <button 
                  ...
                  className="w-8 h-8 flex items-center justify-center rounded-full bg-cyan-600 text-white hover:bg-cyan-500 disabled:opacity-50 transition-colors shrink-0 disabled:bg-surface-hover disabled:text-muted"
                >
