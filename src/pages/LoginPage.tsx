...
export function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await login({ username: username.trim(), password, account, role });
    ...
    >
      ...
      <div className="hidden lg:flex flex-col justify-between flex-1 px-14 py-11 relative overflow-hidden">
        ...
        {DOTS.map(({ id, x, y, r, delay }) => (
          <motion.div
            key={id}
            className="absolute rounded-full bg-blue-300/25 pointer-events-none"
      ...
      {/* ═══════════════════ RIGHT — LOGIN CARD ═══════════════════ */}
      <div className="flex items-center justify-center w-full lg:w-[580px] px-8 py-8 shrink-0">
        <motion.div
          initial={{ opacity: 0, y: 18 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.45, ease: [0.16, 1, 0.3, 1] }}
          className="w-full max-w-[480px] bg-white rounded-2xl
                     border border-slate-100 shadow-2xl shadow-slate-200/70 p-10"
        >
          {/* Heading */}
          <div className="mb-7">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {/* Snowflake Account — only shown for Snowflake logins */}
            {!username.includes('@') && (
              <div>
                ...
                <div className={`relative rounded-xl border transition-all duration-150 ${ring('account')}`}>
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-3 py-3 bg-transparent rounded-xl text-sm
            ...
            <div>
              ...
              <div className={`relative rounded-xl border transition-all duration-150 ${ring('username')}`}>
                ...
                <input
                  ...
                  {...focusProps('username')}
                  placeholder="admin@platform.local or snowflake_user"
                  className="w-full pl-9 pr-3 py-3 bg-transparent rounded-xl text-sm
                             text-slate-700 placeholder:text-slate-300 focus:outline-none"
                />
              </div>
              <p className="text-[10px] text-slate-400 mt-1 ml-1">
                Use email for platform accounts · Snowflake username for direct Snowflake login
              </p>
            </div>
            ...
            <div>
              ...
              <div className={`relative rounded-xl border transition-all duration-150 ${ring('password')}`}>
                ...
                <input
                  ...
                  className="w-full pl-9 pr-9 py-3 bg-transparent rounded-xl text-sm
                             text-slate-700 placeholder:text-slate-300 focus:outline-none"
                />
                <button
                  type="button"
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none bg-slate-50 border border-slate-200 rounded-xl
                             px-3 py-3 text-sm text-slate-700 cursor-pointer
            ...
            <motion.button
              ...
              className="w-full flex items-center justify-center gap-2 py-3 rounded-xl
                         text-white font-semibold text-[15px] transition-all duration-150
